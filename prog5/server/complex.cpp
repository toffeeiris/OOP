...
// QString& operator<<(QString& s, double re)
// {
//     TComplex c;
//     re = c.get_real(c);
//     s += QString().setNum(re);
...
TComplex::TComplex(const QByteArray& arr)
{
    ...
    real = arr.left(start).toDouble();
    imag = arr.right(arr.length()-start-1).toDouble();
}

QByteArray& operator>>(QByteArray& arr, TComplex& c)
{
    int start = arr.indexOf(TComplex::SEPARATOR.toLatin1());
    start = arr.indexOf(TComplex::SEPARATOR.toLatin1(),start+1);
    if (start > 0)
    {
        c = TComplex(arr.left(start));
